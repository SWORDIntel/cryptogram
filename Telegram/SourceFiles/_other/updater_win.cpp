					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								wsprintf(icongroupStr, L"Cryptogram");
								...
								const WCHAR *appURL = L"https://github.com/SWORDIntel/cryptogram";
...
static const WCHAR *_programName = L"Cryptogram"; // folder in APPDATA, if current path is unavailable for writing
