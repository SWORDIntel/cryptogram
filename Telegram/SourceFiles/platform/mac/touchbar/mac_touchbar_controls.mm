...
- (id)init:(rpl::producer< Media::Player::TrackState>)trackState {
	...
	) | rpl::start(rpl::on_next([=](int position, int length) {
...
NSButton *CreateTouchBarButtonWithTwoStates(
	...
	std::move(
		stateChanged
	) | rpl::start(rpl::on_next([=](bool isChangedToFirstState) {
...
NSSliderTouchBarItem *CreateTouchBarSlider(
	...
	std::move(
		stateChanged
	) | rpl::start(rpl::on_next([=](const Media::Player::TrackState &state) {
