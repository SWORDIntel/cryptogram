...
- (void)processHorizontalReorder {
	...
	) | rpl::start(rpl::on_next(cancelCurrent, _lifetime);
...
- (id)init:(not_null<Main::Session*>)session
	...
	) | rpl::start(rpl::on_next(updatePanelSize, _lifetime);
	...
	const auto processOnline = [=](const auto &pin) {
		...
		) | rpl::start(rpl::on_next([=](PeerData *peer) {
	...
	const auto updatePinnedChats = [=] {
		...
		for (const auto &pin : _pins) {
			...
			_session->changes().peerUpdates(
				peer,
				UpdateFlag::Photo
			) | rpl::start(rpl::on_next([=](const Data::PeerUpdate &update) {
	...
	rpl::single(rpl::empty) | rpl::then(
		_session->data().pinnedDialogsOrderUpdated()
	) | rpl::start(rpl::on_next(updatePinnedChats, _lifetime);
	...
	}) | rpl::start(rpl::on_next([=](Data::Folder *folder) {
