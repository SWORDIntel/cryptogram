...
QByteArray Settings::serialize() const {
	...
	result.reserve(size);
	{
		...
		<< qint32(_autoJoinCryptogramChannel ? 1 : 0);
...
void Settings::addFromSerialized(const QByteArray &serialized) {
	...
	qint32 autoJoinCryptogramChannel = 1;
	...
	if (!stream.atEnd()) {
		stream >> autoJoinCryptogramChannel;
	}
	if (!stream.atEnd()) {
	...
	_autoJoinCryptogramChannel = (autoJoinCryptogramChannel == 1);
