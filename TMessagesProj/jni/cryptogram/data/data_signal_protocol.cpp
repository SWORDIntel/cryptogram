...
// On Android, QFile/QDir/QJson* are shimmed in qt_shims.h (via desktop_shims.h)
// On desktop, these resolve to the real Qt headers.
#ifndef ANDROID
...
void SignalProtocol::createSession(not_null<PeerData*> peer, const KeyBundle &remoteBundle) {
    ...
    state.sendingChainKey.assign(
        chainKeyData.begin(),
        chainKeyData.begin() + kAesKeySize);
    state.receivingChainKey.assign(
        chainKeyData.begin() + kAesKeySize,
...
void SignalProtocol::rotateSession(not_null<PeerData*> peer, bool forceRotate) {
    ...
    session.sendingChainKey.assign(
        chainKeyData.begin(),
        chainKeyData.begin() + kAesKeySize);
